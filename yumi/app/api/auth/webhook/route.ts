  ...
  try {
    if (eventType === "user.created") {
      ...
      console.log(`Usuario actualizado: ${evt.data.id}`)
        ...
        const idClerk = evt.data?.id
        if (!idClerk) {
            console.error("ID de Clerk no proporcionado para user.deleted")
            return new NextResponse("ID de usuario no válido", { status: 400 })
        }
        await db.delete(usuarios).where(eq(usuarios.idClerk, idClerk))
